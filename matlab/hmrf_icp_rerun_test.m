function [results] = hmrf_icp_rerun_test(results, data, modes)
...
for angle_i=1:size(results.params, 1)
  for frame_pair_i=1:size(results.params, 2)
    ref_frame = results.params{angle_i, frame_pair_i}(1);
    src_frame = results.params{angle_i, frame_pair_i}(2);
    ol = results.params{angle_i, frame_pair_i}(3);
    angle = results.params{angle_i, frame_pair_i}(4);
    axis_i = results.params{angle_i, frame_pair_i}(5);
    axis = axes(axis_i, :)';
